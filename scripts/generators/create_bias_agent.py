...
agent_path = Path("backend/app/ai/agents/bias_detection_agent.py")
...
class BiasDetectionAgent(BaseAgent):
    ...
    def _process(self, state: ReviewState, context: ExecutionContext) -> ReviewState:
        ...
        sys_prompt, user_prompt = self.prompt_registry.render_prompt(
            ...
            variables={"evidence": evidence_text, "employee_name": "Employee", "manager_feedback": "N/A"}
        )
...
sys_bias = Path("backend/app/ai/prompts/system/bias.txt")
sys_bias.parent.mkdir(parents=True, exist_ok=True)
sys_bias.write_text("You are a strict Bias Detector. Only use the provided evidence. Return structured JSON.", encoding="utf-8")

user_bias = Path("backend/app/ai/prompts/user/bias.txt")
user_bias.parent.mkdir(parents=True, exist_ok=True)
user_bias.write_text("Evidence:\\n{{evidence}}\\nEvaluate this for bias.", encoding="utf-8")

print("BiasDetectionAgent created")
